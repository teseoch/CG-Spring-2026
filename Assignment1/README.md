 ...
 * Familiarize yourself with `igl` and the mesh viewer in `pyvista`.
...
*Relevant `igl` functions:* `igl.facet_components`

Call `to_pyvista_mesh(v, f).plot(scalars=c, show_edges=False)`, where `c` is the computed labels to display colors to the per-face colors you computed.
